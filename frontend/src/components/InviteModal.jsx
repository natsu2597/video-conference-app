    ...
    <div className="create-channel-modal-overlay">
        <div className="create-channel-modal">
            ...
            <div className="create-channel-modal__form">
                {isUserLoading && <p>User is Loading ...</p>}
                {error && <p className="form-error">{error}</p>}
                {users.length === 0 && !isUserLoading && <p>No Users Found</p>}

                {users.length > 0 && (
                    users.map((user) => {
                        const isChecked = selectedMembers.includes(user.id);
                        return (
                            <label key={user.id} className={`flex items-center gap-4 p-3 rounded-lg cursor-pointer transition-all shadow-sm bg-white hover:bg-[#f5f3ff] border-2 ${isChecked ? "border-[#611f69] bg-[#f3e6fa]" : "border-gray-700"}`}>
                                <input type="checkbox" className="checkbox checkbox-primary checkbox-sm accent-[#611f69]" 
                                value={user.id}
                                onChange={(e) => {
